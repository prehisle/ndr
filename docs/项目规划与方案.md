# 通用资料管理微服务（DMS）项目规划与方案

> 更新日期：2025-10-19  
> 适用范围：DMS 服务的整体架构、技术决策与迭代规划。  
...
- **对外交互**：提供 REST API；上游完成终端用户认证，DMS 仅实现最小化服务鉴权（API Key，可按环境启停）。
...
| **nodes** | `name`、`slug`、`path (ltree)`、`parent_path`、审计字段、软删字段 | `slug` 构成路径；`ltree` 支撑子树查询；同父下 `name` 唯一 |
...
> 如需在新会话中使用，请同时附上本文件与“当前进度”文档，以便快速建立上下文。
