...
- [13. 运维速查](#13-运维速查)
- [附录](#附录)
  - [A. 环境变量完整列表](#a-环境变量完整列表)
  - [B. API 端点速查](#b-api-端点速查)
  - [C. 常用 SQL 查询](#c-常用-sql-查询)
  - [D. 故障码速查](#d-故障码速查)
  - [E. 联系方式](#e-联系方式)
...
- **软删除**: 默认软删除，支持恢复和永久清除
- **资产不可变**: 上传完成后资产内容不可修改，更新文件需创建新资产
...
默认端口约定：
- 应用宿主机端口：`APP_PORT`（默认 9001，建议同机多项目使用 9001-9009）
- 数据库宿主机端口：`PG_PORT`（默认 5541；容器内仍为 5432）

访问: http://localhost:9001（可通过 `APP_PORT` 调整）
...
# 查看 API 文档
open http://localhost:9001/docs  # 或浏览器访问
```
...
│   │   ├── asset_service.py     # 资产服务 (新增)
│   │   ├── node_asset_service.py # 节点-资产关联 (新增)
│   │   ├── bundle.py            # 服务工厂
│   │   └── base.py              # 基础服务类
│   ├── domain/repositories/     # 数据访问层
│   │   ├── document_repository.py
│   │   ├── node_repository.py
│   │   ├── asset_repository.py  # 资产仓储 (新增)
│   │   ├── node_asset_repository.py # 关联仓储 (新增)
...
├── tests/                       # 测试套件
│   ├── api/
│   │   └── test_assets.py      # 资产 API 测试 (新增)
│   └── services/
│       ├── test_asset_service.py      # 资产服务测试 (新增)
│       ├── test_node_asset_service.py # 关联服务测试 (新增)
│       └── mock_storage.py            # 存储 Mock (新增)
├── scripts/                     # 工具脚本
├── docs/                        # 项目文档
└── docker-compose.yml
```
...
| storage_backend | String | 存储后端 (默认 "s3") |
| bucket | String | 对象存储桶名称 |
| object_key | String | 对象键 (路径，唯一) |
| status | Enum | UPLOADING / READY / ABORTED / DELETED |
| etag | String | 对象 ETag (完成后填充) |
| metadata_ | JSON | 扩展元数据 (含 upload_id 等) |
| created_by / updated_by | String | 审计字段 |

#### NodeAsset 关联模型
| 字段 | 类型 | 说明 |
|------|------|------|
| node_id | BigInteger | 节点 ID (PK, FK) |
| asset_id | BigInteger | 资产 ID (PK, FK) |
| created_by / updated_by | String | 审计字段 |
...
资产功能需要配置 S3 兼容的对象存储服务（AWS S3、MinIO、阿里云 OSS 等）：

```bash
# .env.production
S3_ENDPOINT_URL=https://s3.amazonaws.com    # 或 MinIO: http://minio:9000
S3_REGION=us-east-1
S3_ACCESS_KEY_ID=your-access-key
S3_SECRET_ACCESS_KEY=your-secret-key
S3_BUCKET=ndr-assets
S3_PREFIX=assets/
S3_USE_SSL=true
S3_ADDRESSING_STYLE=path                     # MinIO 推荐使用 path

# 上传限制
STORAGE_MAX_UPLOAD_BYTES=1073741824          # 1GB
STORAGE_PART_SIZE_BYTES=16777216             # 16MB (最小 5MB)
STORAGE_PRESIGN_EXPIRES_SECONDS=900          # 15分钟
```
...
mc mb local/ndr-assets
```

#### 6.0.2 S3/MinIO CORS 配置

前端直传需要对象存储允许跨域 PUT 请求并暴露 ETag 响应头。

> ⚠️ **注意**: CORS 配置（浏览器跨域）与 Bucket Policy（访问权限）是两个独立配置。预签名 URL 方案不需要配置匿名写入权限，只需配置 CORS。

MinIO CORS 配置示例（通过 Console 或 mc 命令）：
...
AWS S3 控制台配置路径：`Bucket > Permissions > Cross-origin resource sharing (CORS)`

> ⚠️ 如果前端 PUT 请求无法获取 ETag 响应头（浏览器 DevTools 中看不到），说明 CORS 的 `ExposeHeaders` 未正确配置。

#### 6.0.3 分片上传流程

```
1. 客户端 → POST /api/v1/assets/multipart/init
   ↓ 返回 asset_id, upload_id, part_size_bytes

2. 客户端 → POST /api/v1/assets/{id}/multipart/part-urls
   ↓ 返回预签名 URL 列表

3. 客户端 → 直接 PUT 到预签名 URL (上传分片)
   ↓ 返回 ETag

4. 客户端 → POST /api/v1/assets/{id}/multipart/complete
   ↓ 提交所有分片的 part_number + etag
...
| 状态 | 说明 | 允许操作 |
|------|------|----------|
| UPLOADING | 上传进行中 | 获取分片 URL、完成、取消 |
| READY | 上传完成，可下载 | 下载、绑定节点、软删除 |
| ABORTED | 上传已取消 | 无 |
| DELETED | 已软删除 | 恢复 (未实现) |

#### 6.0.5 请求头说明

| 端点 | X-User-Id | Idempotency-Key |
|------|-----------|-----------------|
| POST /assets/multipart/init | 必需 | 推荐 (防止重复创建) |
| POST /assets/{asset_id}/multipart/part-urls | 可选 | 不需要 |
| POST /assets/{asset_id}/multipart/complete | 必需 | 推荐 |
| POST /assets/{asset_id}/multipart/abort | 必需 | 不需要 |
| DELETE /assets/{asset_id} | 必需 | 不需要 |
| POST /assets/{asset_id}/batch-bind | 必需 | 不需要 |
| POST /nodes/{node_id}/assets/{asset_id} | 必需 | 不需要 |
| DELETE /nodes/{node_id}/assets/{asset_id} | 必需 | 不需要 |
| GET /assets/{asset_id} | 可选 | 不需要 |
| GET /assets/{asset_id}/download-url | 可选 | 不需要 |
| GET /assets/{asset_id}/bindings | 可选 | 不需要 |
| GET /nodes/{node_id}/assets | 可选 | 不需要 |

#### 6.0.6 清理过期上传

长时间未完成的上传会占用存储空间，建议定期清理：

```sql
-- 查找 24 小时前未完成的上传
SELECT id, filename, metadata_->'multipart'->>'upload_id' as upload_id, created_at
FROM assets
WHERE status = 'UPLOADING'
  AND created_at < NOW() - INTERVAL '24 hours';
```

可编写定时任务调用 abort 端点清理：
```bash
# 示例：清理脚本 (推荐方式，会同时清理存储侧分片)
for asset_id in $(psql -t -c "SELECT id FROM assets WHERE status='UPLOADING' AND created_at < NOW() - INTERVAL '24 hours'"); do
  curl -X POST "http://localhost:9001/api/v1/assets/${asset_id}/multipart/abort" \
    -H "X-User-Id: system"
done
```

> ⚠️ **重要**: 直接 `DELETE FROM assets` 或 `UPDATE status` 不会清理对象存储中的分片，会产生孤儿对象。应优先使用 API 端点进行清理。
...
-- 最大的 10 个文件
SELECT id, filename, pg_size_pretty(size_bytes) as size, created_at
FROM assets
...
1. 检查数据库是否运行:
   ```bash
   # Docker Compose 默认：PG_PORT=5541（如有调整请同步修改）
   pg_isready -h localhost -p "${PG_PORT:-5541}"

   # 本地 PostgreSQL 默认端口（如未使用 Docker Compose）
...
curl -H "X-Api-Key: your-secure-api-key" \
     http://localhost:9001/api/v1/documents

# 高危操作 (purge)
curl -X POST \
     -H "X-Api-Key: your-secure-api-key" \
     -H "X-Admin-Key: your-admin-key" \
     http://localhost:9001/api/v1/documents/123/purge
```
...
scrape_configs:
  - job_name: 'ndr'
    static_configs:
      - targets: ['localhost:9001']  # 如调整了 APP_PORT，请同步修改
...
READY_URL="http://localhost:${APP_PORT}/ready"
...
upstream ndr_backend {
    server 127.0.0.1:9001;  # 与 APP_PORT 保持一致
    # 负载均衡多实例
...
项目提供了一套完整的自动化备份恢复脚本，位于 `scripts/backup/` 目录。
...
# 恢复指定备份
./scripts/backup/restore.sh --file ndr_20250101_030000.sql.gz
...
```bash
# 方式一：使用独立同步脚本
./scripts/sync_backup_scripts.sh

# 方式二：部署时自动同步（deploy_prod.sh 已集成）
...
# 每天凌晨 3:00 执行备份
0 3 * * * /home/dy_prod/ndr9000/scripts/backup/backup.sh >> /home/dy_prod/ndr9000/backups/logs/cron.log 2>&1

# 每周日凌晨 4:00 验证备份
0 4 * * 0 /home/dy_prod/ndr9000/scripts/backup/verify.sh --latest >> /home/dy_prod/ndr9000/backups/logs/verify.log 2>&1
```

#### 配置项
编辑 `scripts/backup/config.sh` 修改配置：
- `BACKUP_RETENTION_DAYS`: 保留天数（默认 7）
- `STORAGE_BACKEND`: 存储后端（local/s3/nas）
- `ENABLE_COMPRESSION`: 是否压缩（默认 true）
...
./scripts/backup/backup.sh --backend s3
```
...
# 7. 验证服务
curl "http://localhost:${APP_PORT:-9001}/ready"
```
...
- 完全清理（会删除 `pgdata` 数据卷）：
  ...
  docker compose down -v
  ```
- 访问与验证（默认端口 `9001`，可通过 `APP_PORT` 调整）：
  ```bash
  curl -fsS http://127.0.0.1:9001/health
  curl -fsS http://127.0.0.1:9001/ready
  curl -fsS http://127.0.0.1:9001/openapi.json > /dev/null
  ```
...
| STORAGE_BACKEND | str | s3 | 存储后端类型 |
| STORAGE_MAX_UPLOAD_BYTES | int | 1073741824 | 最大上传大小 (1GB) |
| STORAGE_PART_SIZE_BYTES | int | 16777216 | 分片大小 (16MB) |
| STORAGE_PRESIGN_EXPIRES_SECONDS | int | 900 | 预签名 URL 有效期 (15分钟) |
| S3_ENDPOINT_URL | str | None | S3 兼容存储端点 (如 MinIO) |
| S3_REGION | str | None | S3 区域 |
| S3_ACCESS_KEY_ID | str | None | 存储访问密钥 |
| S3_SECRET_ACCESS_KEY | str | None | 存储密钥 |
| S3_BUCKET | str | ndr-assets | 资产存储桶名称 |
| S3_PREFIX | str | assets/ | 对象键前缀 |
| S3_USE_SSL | bool | true | 是否使用 SSL |
| S3_ADDRESSING_STYLE | str | path | 地址风格 (path/virtual) |
...
| **资产管理** | | |
| POST | /api/v1/assets/multipart/init | 初始化分片上传 |
| GET | /api/v1/assets/{asset_id} | 获取资产元数据 |
| GET | /api/v1/assets | 分页列表资产 |
| DELETE | /api/v1/assets/{asset_id} | 软删除资产 |
| POST | /api/v1/assets/{asset_id}/multipart/part-urls | 获取分片预签名 URL |
| POST | /api/v1/assets/{asset_id}/multipart/complete | 完成分片上传 |
| POST | /api/v1/assets/{asset_id}/multipart/abort | 取消分片上传 |
| GET | /api/v1/assets/{asset_id}/download-url | 获取下载预签名 URL |
| GET | /api/v1/assets/{asset_id}/bindings | 列出资产绑定的节点 |
| GET | /api/v1/assets/{asset_id}/binding-status | 资产绑定状态统计 |
| POST | /api/v1/assets/{asset_id}/batch-bind | 批量绑定节点 |
| POST | /api/v1/nodes/{node_id}/assets/{asset_id} | 挂载资产到节点 |
| DELETE | /api/v1/nodes/{node_id}/assets/{asset_id} | 解除资产挂载 |
| GET | /api/v1/nodes/{node_id}/assets | 列出节点的资产 |

> ⚠️ 仅调整展示顺序时，请调用 `/api/v1/documents/reorder`，避免 `PUT /api/v1/documents/{id}` 生成新的版本记录。

> ℹ️ `PUT /api/v1/documents/{id}` 在 `metadata` 字段上遵循 JSON Merge Patch 语义：仅更新负载中出现的键，值为 `null` 时删除该字段，未出现的键保持不变。
...
-- 查找上传中超过 24 小时的资产 (可能需要清理)
SELECT id, filename, created_at, metadata_->'multipart'->>'upload_id' as upload_id
FROM assets
WHERE status = 'UPLOADING'
  AND created_at < NOW() - INTERVAL '24 hours';
```
...
- **文档**: 参考 `docs/` 目录下的设计文档
- **API 文档**: http://localhost:9001/docs（如调整了 `APP_PORT` 请同步修改）

---

**文档版本**: 1.1
**最后更新**: 2025-12-22
**适用版本**: NDR v0.2.0+ (含资产管理功能)
