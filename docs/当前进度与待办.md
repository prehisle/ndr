...
- **架构分层**：路由仍直接操作 ORM，会话中已确认需抽象 `NodeService.get_subtree_documents` 等应用/领域服务，并补齐对应单元测试。
- **ltree 基准**：`scripts/benchmark_ltree.py` 可输出 GIST/GIN 指标；若环境缺少 `ltree_gin` 运算符类，会提示跳过 GIN，当前仅落地 GIST 结果。
- **测试金字塔**：API 集成测试与数据库集成测试已覆盖，领域/服务层单测尚未补齐。
...
2. **服务层抽象**：
   - 设计 `NodeService`/`RelationshipService` 等应用层，提供 `get_subtree_documents` 等组合能力。
  - 补充领域层单元测试，并为仓储封装 ltree 查询。
...
- **路由层膨胀风险**：在未拆分服务层前，新功能易堆积到路由函数中，需谨慎控制复杂度。
