...
def upgrade() -> None:
    ...
    if bind.dialect.name == "postgresql":
        try:
            ...
            ImportError
        ):  # pragma: no cover - fallback when ltree dialect is unavailable
            _pg_ltree = None
        else:
    ...
    op.create_index(
        "ix_documents_metadata_gin", "documents", ["metadata"], postgresql_using="gin"
    )
    ...
    index_kwargs: dict[str, object] = (
        {"postgresql_using": "gist"} if ltree_available else {}
    )
